...
public class DotCluster extends DotNode {
	...
	/**
	 *
	 * @return An unmodifiable list of all nodes in the graph (not in sub-graphs).
	 */
	 ...
	 *
	 * @param label
	 * @param options A map of node options, which will not be copied.
	 * @return the new dot node.
	 */
	...
	/**
	 *
	 * @return An unmodifiable list of all edges in this graph (not in sub-graphs).
	 */
	...
	/**
	 * Add an edge that connects source to target, having a label, without further
	 * options set.
	 *
	 * @param source
	 * @param target
	...
	/**
	 * Sets a default edge option. This will be the default for all edges in this
	 * graph.
	 *
	 * @param option
	 * @param value
	...
	/**
	 *
	 * @return An unmodifiable set of all default edge options set for this graph.
	 */
...
//	}
	...
	/**
	 *
	 * @return An unmodifiable list of clusters in this graph (not in sub-graphs).
	 */
...
//	}
	...
	protected void appendOptions(StringBuilder result) {
		for (String key : getOptionKeySet()) {
			/*
			 * HV, May 11, 2017 Graph label may contain HTML code. Deal with that.
			 */
			if (key.equals("label")) {
